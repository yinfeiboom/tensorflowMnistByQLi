...
def mnist_model(learning_rate, use_two_conv, use_two_fc, hparam):
  ...
  if use_two_fc:
    ...
    h_fc1_drop = tf.nn.dropout(fc1, 0.5)
   ...
   # logits = fc_layer(fc1, 1024, 10, "fc2")
    ...
    w = tf.Variable(tf.truncated_normal([7 * 7 * 64, 10], stddev=0.1), name="W")
    b = tf.Variable(tf.constant(0.1, shape=[10]), name="B")
    #h_fc1_drop = tf.nn.dropout(f_fc1, 0.5)
    y_conv = tf.nn.softmax(tf.matmul(flattened, w) + b)
